...
/*
 *将静态定义的(DEFINE_PER_CPU)复制NR_CPUS份，每个cpu一份,
 *DEFINE_PER_CPU静态定义的变量都在.data.percpu section中,
 *这个函数中 重新分配内存，copy.data.percpu的数据到分配的内存
 */
static void __init setup_per_cpu_areas(void)
{
	...
	/*
	 * __per_cpu_end,__per_cpu_start,是在链接脚本中指定的两个变量，
	 * 分别代表.data.percpu section的起始和结束地址
	 */
 ...
 */
	...
	setup_per_cpu_areas();/*处理DEFINE_PER_CPU宏定义的静态每cpu变量*/
...
static int init(void * unused)
{
	...
	smp_prepare_cpus(max_cpus);/*这个函数中会启动其他cpu*/
