...
/*确定新叶子节点在与给定线性地址对应的红黑树中的位置，
 * 并返回前一个线性区的地址和要插入的叶子节点的父节点的地址*/
...
full_search:
	for (vma = find_vma(mm, addr); ; vma = vma->vm_next) {
		...
		if (TASK_SIZE - len < addr) {
			 ...
			 * some holes.
			 */
			if (start_addr != TASK_UNMAPPED_BASE) {/*如果遍历到TASK_SIZE 没有找到 从TASK_UNMAPPED_BASE 开始查找*/
...
/*找到第一个符号mm->end>addr的线性区, addr 不一定包含在该线性区内*/
