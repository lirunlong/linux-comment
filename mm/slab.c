 ...
 */
struct slab {
	/*slab描述符的三个双向循环链表中的一个*/
	struct list_head	list;
	/*slab中第一个对象的偏移*/
	unsigned long		colouroff;
	/*slab中第一个对象*/
	void			*s_mem;		/* including colour offset */
	/*当前正在使用的slab中的对象的个数(非空闲的）*/
	unsigned int		inuse;		/* num of objs active in slab */
	/*slab中下一个空闲对象的下标，如果没有剩下空闲对象，则为BUFCTL_END*/
	kmem_bufctl_t		free;
};
 ...
 */
struct array_cache {
	/*指向本地告诉缓存中可以使用对象的指针的个数,它同时也作为高速缓存中第一个空槽的下标*/
	unsigned int avail;
	/*本地高速缓存的大小，也就是本地高速缓存中指针的最大个数*/
	unsigned int limit;
	/*本地高速缓存重新填充或腾空时使用的块大小*/
	unsigned int batchcount;
	/*如果本地高速缓存最近已经被使用过，则该标志设为1*/
...
/* 1) per-cpu data, touched during every alloc/free */
	...
	/*要移进本地告诉缓存或从本地高速缓存中转移出的大批对象的数量*/
	unsigned int		batchcount;
	/*本地高速缓存中空闲对象的最大数目(可调的)*/
	unsigned int		limit;
/* 2) touched by every alloc & free from the backend */
	...
	/*封装在一个单独slab中的对象的个数(告诉缓存中的所有slab具有相同的大小)*/
...
/* 3) cache_grow/shrink */
	...
	/*指针指向包含slab描述符的普通的slab告诉缓存(如果使用来内部slab描述符，则这个字段为null)*/
		 ...
		 int flags, size_t *left_over, unsigned int *num)
{
	...
	/*如果为内部slab描述符，则开始存储一个slab描述符*/
	if (!(flags & CFLGS_OFF_SLAB)) {
		base = sizeof(struct slab);/*slab描述符的大小*/
		extra = sizeof(kmem_bufctl_t);/*对象描述符的大小， 一个slab中对象描述符的总大小为*n  一个obj  一个对象描述符*/
	}
	i = 0;
	while (i*size + ALIGN(base+i*extra, align) <= wastage)
		i++;
	if (i > 0)/*while 循环中  实际只能村下i-1个  所以这里做i--*/
		i--;
	...
	*num = i;
	...
	/*
	 *slab kmem_bufctl_t*i align  obj*i  
	 */
	*left_over = wastage;
}
...
/*
 * 在start_kernel中调用，
 * 初始化cache_cache 高速缓存(高速缓存描述符kmem_cache_s)
 * 初始化malloc_sizes表中的通用slab 大小为32, 64.128,等13个
 */
void __init kmem_cache_init(void)
{
	...
	list_add(&cache_cache.next, &cache_chain);/*将cache_cache插入cache_chain链表*/
	cache_cache.colour_off = cache_line_size();
	cache_cache.array[smp_processor_id()] = &initarray_cache.cache;

	cache_cache.objsize = ALIGN(cache_cache.objsize, cache_line_size());

	/*估算一个单独slab中对象的个数，left_over用于求slab着页初始话*/
	cache_estimate(0, cache_cache.objsize, cache_line_size(), 0,
				&left_over, &cache_cache.num);
	if (!cache_cache.num)
		BUG();

	/*slab中颜色为 剩余的空间/要求对齐的偏移，
	 * 那么 第一个slab的偏移就是0  第2个就是colour*1，一次类推
	 * 这样  不同slab中的对象就可以映射到高速缓存中的不同行
	 */ 
...
static void *kmem_getpages(kmem_cache_t *cachep, int flags, int nodeid)
{
	...
	i = (1 << cachep->gfporder);
	/*如果SLAB_RECLAIM_ACCOUNT置位，则增加可回收的页框数*/
...
static void kmem_freepages(kmem_cache_t *cachep, void *addr)
{
	...
	/*如果当前进程正在回收内存， 则在reclaimed_slab字段上+上释放的页框数*/
	if (current->reclaim_state)
		current->reclaim_state->reclaimed_slab += nr_freed;
	free_pages((unsigned long)addr, cachep->gfporder);
	/*如果SLAB_RECLAIM_ACCOUNT标志置位，slab_reclaim_pages适当改变*/
	if (cachep->flags & SLAB_RECLAIM_ACCOUNT) 
...
retry:
	...
	BUG_ON(ac->avail > 0);
	spin_lock(&cachep->spinlock);
	/*如果共享cpu告诉缓存存在， 则尝试重共享cpu高速缓存中获取*/
	if (l3->shared) {
	...
	/* 尝试从 partial和must_grow链表中填充*/
	while (batchcount > 0) {
		struct list_head *entry;
		struct slab *slabp;
		/* Get slab alloc is to come from. */
		entry = l3->slabs_partial.next;
		/*检查partial 和free链表  入股都不存在， 则直接跳转到mast_grow*/
		...
		list_del(&slabp->list);
		if (slabp->free == BUFCTL_END)/*如果当前slab所有的对象都分配出去，则插入到full链表*/
