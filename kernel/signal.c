...
#define sig_fatal(t, signr) \
 ...
 /*1.如果进程没有被跟踪，2信号没有被阻塞，显示的忽略信号 或者隐含的忽略信号，则函数返回1
  * 信号忽略*/
...
void signal_wake_up(struct task_struct *t, int resume)
{
	...
	if (resume)/*如果是sigkill  信号 才能唤醒task_stopped 和task_traced 状态的进程*/
		mask |= TASK_STOPPED | TASK_TRACED;
	if (!wake_up_state(t, mask))
		/*返回0 说明已经在其他cpu上开始运行， 发送一个ipi ，强制当前进程重新调度，因为从调度
		 * 函数返回时，每个进程都检查是否存在挂起的信号，因此，处理器间中断保证来目标进程能很快的注意到新的挂起信号*/
		kick_process(t);
}
				 ...
				 struct task_struct *t)
{
	...
	error = -EPERM;
	/*如果信号是从用户态进程发送的，
	 * 1.发送进程必须具有适当的全能(capable(cap_kill) 可以发送)
	 * 2.信号为sigcont，并且目标进程与发送进程同处于一个注册会话中
	 * 3.两个进程同属于一个用户，
	 * 至少一个条件成立才可以发送
	 */
	if ((!info || ((unsigned long)info != 1 &&
			(unsigned long)info != 2 && SI_FROMUSER(info)))
			/*上面两行检测如果信号是从用户太进程发送的*/
	    && ((sig != SIGCONT) ||
 ...
 */
/*检查信号的某些类型，这些类型可能使目标线程组的其他挂起信号无效。*/
static void handle_stop_signal(int sig, struct task_struct *p)
{
	...
	if (sig_kernel_stop(sig)) {
		 ...
		 */
		/*如果是一个stop信号，则从共享信号队列 和所有的线程里的私有队列里删除所有的sigcont信号*/
			...
			struct sigpending *signals)
{
	 ...
	 * or SIGKILL.
	 */
	/*如果是2，这个信号是sigkill或sigstop，而且已经由内核通过force_sig_specific()函数产生，因此，直接跳转到out_set
	 * 内核立即强制执行与这些信号相关的操作，因此函数不用把信号添加到挂起信号队列中*/
	...
	if (q) {/*分配成功，插入对因的sigpending 的list中*/
...
/*@info:0意味着信号是由用户态进程发送的，1意味着是由内核发送的，2意味着是由内核发送的SIGSTOP或SIGKILL信号*/
/*必须在本地中断已经获得t->sighand->siglock 和本地中断禁止的情况下调用*/
static int
specific_send_sig_info(int sig, struct siginfo *info, struct task_struct *t)
{
	...
	if (((unsigned long)info > 2) && (info->si_code == SI_TIMER))/*如果定时器到期信号*/
		/*
	...
	/* Support queueing exactly one non-rt signal, so that we
	   ...
	   the signal. */
	/*检查进程是否是非实时信号，如果是，则如果在t->pendding 已经存在这个信号 则直接返回*/
	if (LEGACY_QUEUE(&t->pending, sig))
		goto out;

	ret = send_signal(sig, info, t, &t->pending);
	/*如果send_signal返回成功，并且sig没有被阻塞，则调用signal_wake_up函数通知进程有新的挂起信号*/
...
__group_complete_signal(int sig, struct task_struct *p)
{
	...
	/*选择一个线程来接受信号,从p->signal->curr_target开始*/
...
int group_send_sig_info(int sig, struct siginfo *info, struct task_struct *p)
{
	...
	/*如果sig 为0，用户发送进程检查它是否有向目标线程组发送信号的特权,如果目标进程被杀死(通过检查他的信号处理程序描述符是否已经释放)
	 * 函数立即返回
	 */
...
static int kill_something_info(int sig, struct siginfo *info, int pid)
{
	if (!pid) {
		...
		/*信号发到所有有权限的用户，init,swapper ,current 除外*/
	...
	for (;;) {
		...
		if (ka->sa.sa_handler != SIG_DFL) {
			...
			*return_ka = *ka;
			...
			/*如果所接受信号的SA_ONESHOT标志被设置，就必须重新设置它的缺省操作*/
		...
		if (sig_kernel_stop(signr)) {
			 ...
			 * We need to check for that and bail out if necessary.
			 */
			/*sigstop信号总是停止线程组，其他信号只停止不在“孤儿进程组”中的线程组*/
			if (signr != SIGSTOP) {
				spin_unlock_irq(&current->sighand->siglock);
