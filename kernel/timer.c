...
del_again:
	...
	smp_rmb();
	if (timer_pending(timer))/*定时器函数可能重新执行add_timer*/
		goto del_again;
...
static inline void __run_timers(tvec_base_t *base)
{
	...
	while (time_after_eq(jiffies, base->timer_jiffies)) {/*如果再次发生时钟中断处理程序，jiffies值会增加，所以jiffies不能在这里保存为局部变量*/
		...
		/*如果index==0 base->tv1.vec+index中的是从tv2新拆开放进去的*/
repeat:
		if (!list_empty(head)) {
			...
			spin_unlock_irq(&base->lock);
			{
				...
				fn(data);
				if (preempt_count != preempt_count()) {/*定时器函数中对preempt_count修改不成对*/
	...
	set_running_timer(base, NULL);/*执行完后清楚running_timer字段*/
	spin_unlock_irq(&base->lock);
}
...
void update_process_times(int user_tick)
{
	...
	run_local_timers();/*本地cpu上的TIMER_SOFTIRQ任务队列*/
	if (rcu_pending(cpu))
 ...
 */
/*在timer_interrupt(全局时钟中断)->do_timer_interrupt()->do_timer_interrupt_hook()->do_timers()->update_timers()->calc_load()中更新
 * 平均系统负载*/
...
static inline void calc_load(unsigned long ticks)
{
	...
	if (count < 0) {/*每5秒统计一次*/
		count += LOAD_FREQ;
		active_tasks = count_active_tasks();/*统计TASK_RUNNING,TASK_UNINTERRUPTIBLE状态的进程*/
...
static inline void update_times(void)
{
	...
	if (ticks) {/*如果有丢失的时钟中断，则ticks>1,从mark_offset中检测到*/
		wall_jiffies += ticks;
