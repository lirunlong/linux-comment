...
struct runqueue {
	 ...
	 */
	/*先前在运行队列链表中而现在睡眠在TASK_UNINTERRUPTIBLE状态的进程的数量(对所有运行队列来说，只有这些字段总数才有意义的*/
	...
	/*
	 * curr:当前正在运行的进程的进程描述符
	 * idle:当前cpu上的swapper进程的进程描述符
	 */
	task_t *curr, *idle;
	/*在进程切换期间用来存放被替换进程的内存描述符的指针
	 * 进程切换时候，如果prev是一个内核线程，会将他使用能的内存描述符赋给prev_mm
	 * 在finish_task_switch 时候会做处理 mmdrop(mm);
	 * */
	struct mm_struct *prev_mm;
	prio_array_t *active, *expired, arrays[2];
	/*过期进程中静态优先级最高的进程*/
	int best_expired_prio;
	/*先前在运行队列链表中 现在正在等待磁盘I/O操作结束的进程的数量*/
	atomic_t nr_iowait;
	...
	/*如果要把一些进程从本地运行队列迁移到另外的运行队列（平衡运行队列），接设置这个标志*/
	int active_balance;
	int push_cpu;
...
static void recalc_task_prio(task_t *p, unsigned long long now)
{
	...
	if (likely(sleep_time > 0)) {
		 ...
		 */
		/*p->mm：不是内核线程
		 * p->activated!=-1:不是从uninterruptible状态唤醒的
		 *睡眠时间超过睡眠时间极限
		 *then 把sleep_avg设置为900个节拍
		 */
...
static int wake_idle(int cpu, task_t *p)
{
	...
	if (idle_cpu(cpu))/*如果当前cpu空闲，之间返回当前cpu的id*/
		return cpu;
...
static int try_to_wake_up(task_t * p, unsigned int state, int sync)
{
	...
	old_state = p->state;
	/*如果p的状态不属于要唤醒的状态 在直接返回*/
	if (!(old_state & state))
		goto out;
	/*如果p已经在运行队列中，则直接更新状态为running状态  返回*/
	if (p->array)
		goto out_running;
	...
	/*如果最近运行的cpu是当前的cpu  或者是进程的cpu掩码没有设置当前cpu
	 * 则尝试选择一个空闲的cpu 如果没有空闲的cpu  则重新在原来的cpu上运行
	 * */
	...
	/*
	 *如果最近运行的cpu不是当前cpu  而且当前进程也设置里当前cpu掩码,则尝试选择当前cpu
	 */
	...
	/*如果load<SCHED_LOAD_SCALE/2 说明load 原来的cpu空闲(rq->cpu_load设置的时候为两个load值相加/2  所以可能小于1个SCHED_LOAD_SCALE
	 * 这个时候不能选择当前cpu  因为当前cpu至少有一个运行进程*/
...
out_set_cpu:
	...
	new_cpu = wake_idle(new_cpu, p);/*如果没有找到空闲的cpu  则返回new_cpu*/
	if (new_cpu != cpu) {/*选择 了一个空闲cpu*/
		schedstat_inc(rq, ttwu_moved);
		set_task_cpu(p, new_cpu);
		task_rq_unlock(rq, &flags);
		/* might preempt at this point */
		rq = task_rq_lock(p, &flags);
		old_state = p->state;/*可能会发生竞争  所以重新判断状态*/
		if (!(old_state & state))
			goto out;
		if (p->array)
			goto out_running;
...
{
	runqueue_t *rq = this_rq();
	struct mm_struct *mm = rq->prev_mm;/*内核线程mm不是null*/
...
task_t * context_switch(runqueue_t *rq, task_t *prev, task_t *next)
{
	...
	if (unlikely(!mm)) {/*next为内核线程，没有自己的内存描述符，使用前一个进程的内存描符*/
		next->active_mm = oldmm;
		atomic_inc(&oldmm->mm_count);
		/*如果为smp 则设置为懒惰tlb模式,不接受所有的刷新tlb的cpu间中断请求*/
		enter_lazy_tlb(oldmm, next);
	} else
		switch_mm(oldmm, mm, next);

	/*如果前一个进程为内核线程  则清除active_mm*/
	if (unlikely(!prev->mm)) {
		prev->active_mm = NULL;
		WARN_ON(rq->prev_mm);
		rq->prev_mm = oldmm;
			...
			struct sched_domain *sd, enum idle_type idle)
{
	...
	if (!nr_moved) {
		...
		if (unlikely(sd->nr_balance_failed > sd->cache_nice_tries+2)) {
			...
			spin_lock(&busiest->lock);
			if (!busiest->active_balance) {/*如果拉不过来 就让最忙的推 在迁移内核线程中做*/
...
/*时钟中断程序调用  多处理器上 是本地apic定时器 239号中断调用,update_process_times->scheduler_tick->rebalance_tick
 * sched_fork也会调用shceduler_tick
 * */
...
static inline void account_it_virt(struct task_struct * p, cputime_t cputime)
{
	...
	/*
	 * it_virt_value表示每次定时器还剩多少时间，
	 *如果it_virt_value<=cputime,则认为定时器时间已到，it_virt_value-cputime<=0,然后从新把it_virt_value设置为p->it_virt_incr,
	 * 发送两次信号的时间间隔
	 *  it_virt_value减去经过的时间cputime
	 * */
...
void account_user_time(struct task_struct *p, cputime_t cputime)
{
	...
	check_rlimit(p, cputime);/*检查当前cpu运行时间是否超标，如果超过cur 发送sigxcpu信号，如果超过max  发送sigkill信号*/
	account_it_virt(p, cputime);/*检查定时器，setitimer(ITIMER_VIRTUAL)*/
	account_it_prof(p, cputime);/*检查定时器，setitimer(ITIMER_PROF)*/

	/* Add user time to cpustat. */
	tmp = cputime_to_cputime64(cputime);
	if (TASK_NICE(p) > 0)
		cpustat->nice = cputime64_add(cpustat->nice, tmp);/*nice值>0的进程的用互态占用时间*/
	else
		cpustat->user = cputime64_add(cpustat->user, tmp);/*nice<=0的用户态占用的时间*/
}
			 ...
			 cputime_t cputime)
{
	...
	if (likely(p->signal && p->exit_state < EXIT_ZOMBIE)) {/*判断是否是代表普通进程或者没有退出的进程，内核县城和idle不算在内*/
		check_rlimit(p, cputime);
		account_it_prof(p, cputime);/*setitimer(ITIMER_PROF)不检查ITIMER_VIRTUAL，这个参数是代表用户太时间的  不保护系统时间*/
	}

	/* Add system time to cpustat. */
	tmp = cputime_to_cputime64(cputime);
	if (hardirq_count() - hardirq_offset)/*因为次函数是在中断中处理程序中执行，所以减去一个hardirq_offset*/
...
void scheduler_tick(void)
{
	...
	if (rt_task(p)) {
		 ...
		 */
		/*如果一个RR实时进程用完他的时间片，则重新计算一个，设置重新调度标志，把当前进程插入活动队列的末尾*/
	...
	if (!--p->time_slice) {
		 ...
		 *如果最老的过期进程已经等待来很长时间，或者过期进程
		 *比交互式进程的静态优先级高，调度程序就把用完时间片
		 *的交互式进程插入过期队列
		 */
		if (!TASK_INTERACTIVE(p) || EXPIRED_STARVING(rq)) {
			enqueue_task(p, rq->expired);
			if (p->static_prio < rq->best_expired_prio)/*如果p的静态优先级比过期队列中最高的静态优先级还高，则更新*/
		 ...
		 */
		/*基本上高优先级的进程会被分成大小TIMESLICE_GRANULARITY(p)的几个片段，使进程不会独占cpu*/
...
static inline int dependent_sleeper(int this_cpu, runqueue_t *this_rq)
{
	...
	if (!(sd->flags & SD_SHARE_CPUPOWER))/*逻辑cpu*/
		return 0;
	 ...
	 */
	if (!this_rq->nr_running)/*上面开始释放了 this_rq->lock,可能产生竞争，所以重新判断*/
		goto out_unlock;
...
need_resched_nonpreemptible:
	...
	if (unlikely(prev->flags & PF_DEAD))/*如果prev正在退出*/
		prev->state = EXIT_DEAD;

	/*nvcsw:自愿的上下文切换 schedule()是直接调用*/
	/*nivcsw：非自愿的上下文切换，内核抢占*/
	switch_count = &prev->nivcsw;
	if (prev->state && !(preempt_count() & PREEMPT_ACTIVE)) {
		switch_count = &prev->nvcsw;
		if (unlikely((prev->state & TASK_INTERRUPTIBLE) &&
				unlikely(signal_pending(prev))))
			/*如果在等待某一个信号来唤醒  碰巧在设置状态的时候  信号已经产生，  而且正好就只有这一次 ，如果移除队列  就永远也没法唤醒，
			 * 参看 sys_sigsuspend*/
...
go_idle:
	...
	if (!rt_task(next) && next->activated > 0) {
		...
		/*交互进程一般是被中断和可延迟函数唤醒(activated==2),所以加上全部的等待时间 来提高动态优先级*/
		if (next->activated == 1)
