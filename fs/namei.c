...
static inline void follow_dotdot(struct vfsmount **mnt, struct dentry **dentry)
{
	while(1) {
		...
		if (*dentry != (*mnt)->mnt_root) {/*如果dentry  不是当前已安装文件系统的根目录 则它的父dentry为dentry->d_parent*/
		...
		spin_unlock(&dcache_lock);
		/*如果dentry为当前已安装文件系统的根目录*/
		spin_lock(&vfsmount_lock);
		parent = (*mnt)->mnt_parent;
		if (parent == *mnt) {/*说明 *mnt 为根所在的文件系统*/
			spin_unlock(&vfsmount_lock);
			break;
		}
		mntget(parent);
	...
	for(;;) {
		...
		if (this.name[0] == '.') switch (this.len) {
			...
			case 2:	
				...
				inode = nd->dentry->d_inode;
				/* fallthrough */
			case 1:/*一个点代表当前目录  所以这个点没效果  继续下一个分量*/
				continue;
		}
		/*
		...
		if (inode->i_op->follow_link) {/*如果为符号链接*/
			mntget(next.mnt);
...
last_component:
		...
		if ((lookup_flags & LOOKUP_FOLLOW)
		    && inode && inode->i_op && inode->i_op->follow_link) {/*分量是一个必须要解释的符号链接*/
			mntget(next.mnt);
			err = do_follow_link(next.dentry, nd);
			dput(next.dentry);
			mntput(next.mnt);
		...
		if (lookup_flags & LOOKUP_DIRECTORY) {
			err = -ENOTDIR; 
			if (!inode->i_op || !inode->i_op->lookup)/*如果最互一个分量要求必须是一个目录  ，则返回-ENOTDIR,目录i_op  i_op->lookup不为NULL*/
				break;
		}
		goto return_base;
lookup_parent:
