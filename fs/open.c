...
repeat:
	...
	if (error) {/*如果发生展开， 则需要repeat   因为展开的过程有释放自旋锁*/
		/*
	 	 * If we needed to expand the fs array we
		 * might have blocked - try again.
		 */

		/*为什么要try again呢，可能是在expand_files返回>0的流程中，因为释放了files->file_lock，可能产生竞争，其他执行流可能关闭了一个文件，
		 *所以我们现在打开就要用一个比较小的文件描述符或者是fd可能被另外一个执行流给占用*/
		error = -EMFILE;
		goto repeat;
	}
	...
	/*put_unused_fd函数关掉一个文件的时候会将next_fd设置为一个比较小的值，所以打开的文件 描述符为最小的 未用的fd,不同与pid*/
	files->next_fd = fd + 1;
#if 1
	/* Sanity check */
	if (files->fd[fd] != NULL) {
