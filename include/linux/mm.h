...
/*线性区是特殊的(如：它映射某个设备的I/O地址空间),因此它的页不能交换出去*/
...
struct page {
	/*一组标志，也对页框所在的管理区进行编号
	 * 高位存放管理区的编码和节点的编码（对于numa会有节点编码）
	 * page_zone(page)返回管理区编码(具体编码规则，flags长度和体系结构有关）
	 * */
	page_flags_t flags;		/* Atomic flags, some possibly
					 * updated asynchronously */
	/*页框的引用计数，如果该字段为-1，相应页框空闲，并可被分配给任一进程或页本身，>=0不空闲，page_count()函数返回页的使用者的数目*/
	...
	/*包含页的最近最少使用双向链表指针,如果页框空闲，由伙伴系统作为链接指针链在不同的空闲块中*/
