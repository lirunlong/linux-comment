...
#define __unlazy_fpu( tsk ) do { \
	/*
	 *如果tsk本次使用过fpu,mmx,sse,ss2指令，则保存这些寄存器的值到tsk->thread->i387字段,clear tsk->thread_info->status的TS_USEDFPU位
	 *并且设置cr0的ts位(由stts()完成，如果下个进程执行需要用到fpu,mmx,sse ss2指令，则会处罚一个devide not available异常)
	 */
